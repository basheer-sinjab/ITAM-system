...
function Dashboard() {
  ...
  const assetInfo = (assetId: string) => assets.find((asset: any) => asset.id === assetId);
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-[1.35fr_0.85fr]">
        <div className="surface-panel overflow-hidden">
          ...
          {maintenance.length ? <div className="divide-y">{maintenance.slice(0, 5).map((record: any) => { const asset = assetInfo(record.asset_id); return <Link key={record.id} to="/maintenance" className="flex items-center justify-between gap-4 p-4 transition-colors hover:bg-muted/50"><div className="flex min-w-0 items-center gap-3"><span className={`flex size-9 shrink-0 items-center justify-center rounded-md ${record.status === "Open" ? "bg-amber-500/10 text-amber-700" : "bg-emerald-500/10 text-emerald-700"}`}><Wrench className="size-4" /></span><div className="min-w-0"><p className="truncate text-sm font-medium">{asset?.name ?? "أصل غير معروف"}{asset?.asset_id && <span className="mr-2 font-mono text-xs text-muted-foreground">({asset.asset_id})</span>}</p><p className="truncate text-xs text-muted-foreground">{record.resolution || record.maintenance_type || "بدون حل"}</p></div></div><div className="shrink-0 text-left"><span className={record.status === "Open" ? "rounded-md bg-amber-500/10 px-2 py-1 text-xs text-amber-700" : "rounded-md bg-emerald-500/10 px-2 py-1 text-xs text-emerald-700"}>{record.status === "Open" ? "مفتوحة" : "مغلقة"}</span><p className="mt-1 text-xs text-muted-foreground">{formatDate(record.maintenance_date)}</p></div></Link>; })}</div> : <EmptyPanel icon={Wrench} title="لا توجد أعمال صيانة مسجلة" description="ستظهر هنا آخر الأعمال فور إنشاء سجل صيانة." to="/maintenance" action="إضافة سجل صيانة" />}
        </div>
