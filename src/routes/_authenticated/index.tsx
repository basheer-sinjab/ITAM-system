...
function Dashboard() {
  ...
  const lowStock = inventory.filter(
    (item: any) => Number(item.quantity) <= lowStockThreshold,
  );
      ...
      <section className="grid gap-4 lg:grid-cols-[1.35fr_0.85fr]">
        <div className="surface-panel overflow-hidden">
          <div className="flex items-center justify-between border-b p-5">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="font-semibold">آخر أعمال الصيانة</h2>
                <p className="text-xs text-muted-foreground">
                  السجلات الأحدث على الأصول
                </p>
              </div>
            </div>
            <Link
                ...
                return (
                  ...
                  >
                    ...
                    <div className="shrink-0 text-left">
                      ...
                      >
                        {record.status === "Open" ? "مفتوحة" : "مغلقة"}
                      </span>
        ...
        <div className="surface-panel p-5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-semibold">صحة بيئة التقنية</h2>
              <p className="text-xs text-muted-foreground">
                مؤشر سريع للأصول النشطة
              </p>
            </div>
          </div>
          <div className="mt-7 flex items-end justify-between">
            <div>
              <p className="text-4xl font-bold">{healthPercent}%</p>
              <p className="mt-1 text-sm text-muted-foreground">أصول نشطة</p>
            </div>
            <span className="flex size-12 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-700">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 border-t pt-5">
            <div>
              <p className="text-lg font-semibold">{activeAssets}</p>
              <p className="text-xs text-muted-foreground">نشطة</p>
            </div>
            <div>
              <p className="text-lg font-semibold">
                {Math.max(0, assets.length - activeAssets)}
              </p>
        ...
        <div className="surface-panel overflow-hidden">
          <div className="flex items-center justify-between border-b p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold">تنبيهات تحتاج انتباه</h2>
            </div>
            ...
            {!alertsEnabled ? (
              ...
              <>
                {lowStock.slice(0, 3).map((item: any) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="flex size-8 items-center justify-center rounded-md bg-rose-500/10 text-rose-700">
                ...
                {expiringLicenses.slice(0, 3).map((license: any) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="flex size-8 items-center justify-center rounded-md bg-amber-500/10 text-amber-700">
                ...
                {expiringWarranties.slice(0, 3).map((asset: any) => (
                  <Link
                    key={asset.id}
                    to="/assets/$id"
                    params={{ id: asset.id }}
...
function DashboardCard({
  ...
  to: "/assets" | "/maintenance" | "/inventory" | "/licenses";
...
function QuickAction({
  ...
  to: "/assets" | "/maintenance" | "/inventory" | "/people-departments";
  icon: React.ElementType;
  title: string;
  description: string;
}) {
...
function EmptyPanel({
  ...
  const content = (
    <div className="flex flex-col items-center justify-center gap-2 px-5 py-9 text-center">
      ...
      {action && (
        <span className="mt-1 text-xs font-medium text-primary">{action}</span>
      )}
    </div>
  );
  return to ? (
...
function useDashboard() {
  return useQuery({
    queryKey: ["dashboard"],
    queryFn: async () => {
      const [assets, maintenance, inventory, licenses, settings] =
        await Promise.all([
