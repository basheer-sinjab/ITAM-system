 ...
 * layer keeps that UI intact while storing all records and images in the
 * browser's IndexedDB database. Nothing is sent to a remote service.
 */
type Row = Record<string, any>;
type Result = { data: any; error: Error | null };

const DATABASE = "printers-manager-local";
const TABLES = [
...
async function put(table: string, value: Row) {
  const db = await openDb();
...
class Query {
  ...
  async execute(): Promise<Result> {
    try {
      ...
      const matches = (r: Row) => this.filters.every(([k, v]) => r[k] === v);
      if (this.operation === "insert") {
        ...
        const item = defaults(this.table, this.payload!); await put(this.table, item); rows = [item];
      } else if (this.operation === "update") {
        rows = rows.filter(matches).map((row) => ({ ...row, ...this.payload, updated_at: this.table === "printers" || this.table === "toners" ? now() : row.updated_at }));
        for (const row of rows) await put(this.table, row);
      } else if (this.operation === "delete") {
        rows = rows.filter(matches); for (const row of rows) await remove(this.table, row.id);
      } else rows = rows.filter(matches);
      if (this.operation === "select") {
        if (this.ordering) { const [key, asc] = this.ordering; rows.sort((a, b) => String(a[key] ?? "").localeCompare(String(b[key] ?? "")) * (asc ? 1 : -1)); }
  ...
  private async expand(rows: Row[]) {
    if (this.table === "toner_replacements" && this.selectText.includes("toner_replacement_items")) {
      const items = await all("toner_replacement_items"); rows = rows.map((r) => ({ ...r, toner_replacement_items: items.filter((i) => i.replacement_id === r.id) }));
    }
    if ((this.table === "toner_replacements" || this.table === "maintenance_records") && this.selectText.includes("printers(")) {
      const printers = await all("printers"); rows = rows.map((r) => ({ ...r, printers: printers.find((p) => p.id === r.printer_id) ?? null }));
    }
    return rows;
  }
...
export const supabase: any = {
  ...
  storage: { from: () => ({ async upload(_path: string, file: File) { const id = uuid(); await put("files", { id, file }); return { data: { path: id }, error: null }; }, async createSignedUrl(path: string) { const files = await all("files"); const file = files.find((x) => x.id === path)?.file; return { data: file ? { signedUrl: URL.createObjectURL(file) } : null, error: file ? null : new Error("الصورة غير موجودة") }; } }) },
};
