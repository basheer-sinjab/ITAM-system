...
function defaults(table: string, value: Row): Row {
  ...
  if (table === "license_assignments")
    return { assignment_date: new Date().toISOString().slice(0, 10), ...base };
...
async function applyInsertDefaults(table: string, payload: Row | Row[]) {
  ...
  for (const value of values) {
    ...
    asset.asset_id ||= nextAssetId(asset.asset_type, [
      ...knownAssets,
      ...prepared,
    ]);
    prepared.push(asset);
...
class Query {
  ...
  constructor(
    ...
    private operation:
      "select" | "insert" | "update" | "delete" | "upsert" = "select",
  ...
  then<TResult1 = Result, TResult2 = never>(
    ...
    fail?: ((reason: any) => TResult2 | PromiseLike<TResult2>) | null,
  ) {
...
export const supabase: any = {
  from(table: string) {
    return {
      select: (text?: string) => new Query(table).select(text),
      insert: (value: Row | Row[]) => new Query(table, "insert", value),
      update: (value: Row) => new Query(table, "update", value),
      delete: () => new Query(table, "delete"),
      upsert: (value: Row) => new Query(table, "upsert", value),
    };
  },
};
