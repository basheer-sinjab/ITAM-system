		...
		configureServer(srv) {
			...
			if (transportMode) {
				...
				srv.middlewares.use((req, res, next) => {
					...
					const count = swallowedFrames;
					swallowedFrames = 0;
					if (count > 0 && wrappedSends.length > 0) wrappedSends[0].original({ type: "full-reload" });
					const body = JSON.stringify({
						flushed: [],
						count,
						mode: "transport-full-reload"
					});
