...
//#region node_modules/.nitro/vite/services/ssr/assets/assets.index-B3vgdAaF.js
...
function AssetsPage() {
	...
	const { data: legacy = [] } = useQuery({
		queryKey: ["legacy-printers"],
		queryFn: async () => (await supabase.from("printers").select("*")).data ?? []
	});
	(0, import_react.useEffect)(() => {
		const missing = legacy.filter((printer) => !assets.some((asset) => asset.legacy_printer_id === printer.id));
		if (missing.length) supabase.from("assets").insert(missing.map((p) => ({
			legacy_printer_id: p.id,
			asset_id: p.asset_id,
			name: p.name,
			asset_type: "Printer",
			...
			assigned_employee_id: p.responsible_person_id,
