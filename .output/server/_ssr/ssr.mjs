...
async function rows(table) {
	definition(table);
	return (await db()).prepare(`SELECT * FROM ${table}`).all().map((row) => hydrate(row));
}
...
function addInventoryMovement(database, itemId, movementType, quantity, movementDate, note) {
	database.prepare("INSERT INTO inventory_movements (id, item_id, movement_date, movement_type, quantity, note, maintenance_id, created_at) VALUES (?, ?, ?, ?, ?, ?, NULL, ?)").run(crypto.randomUUID(), itemId, movementDate, movementType, quantity, note, (/* @__PURE__ */ new Date()).toISOString());
}
...
async function runHardwareAction(request) {
	...
	try {
		if (request.action === "install-toner") {
			...
			addInventoryMovement(database, String(item.id), "use", quantity, installedAt, `تركيب حبر في ${asset.name}`);
		...
		if (request.action === "undo-toner") {
			...
			addInventoryMovement(database, String(installation.inventory_item_id), "return", Number(installation.quantity), today, `تراجع عن تركيب حبر ${installation.toner_name}`);
		...
		if (request.action === "install-part") {
			const asset = recordFor(database, "assets", request.assetId);
			const item = recordFor(database, "inventory_items", request.itemId);
			if (!asset || !isAssetType(asset, "pc")) throw new Error("تركيب القطع متاح لأصول الكمبيوتر المكتبي فقط");
			...
			if (request.oldInstallationId) {
				...
				if (request.oldPartAction === "return_to_stock" && oldPart.inventory_item_id) {
					...
					addInventoryMovement(database, String(oldPart.inventory_item_id), "return", 1, installedAt, `إرجاع قطعة ${oldPart.part_name} بعد استبدالها`);
				}
			...
			addInventoryMovement(database, String(item.id), "use", 1, installedAt, oldPart ? `تركيب ${item.name} بدل ${oldPart.part_name} في ${asset.name}` : `تركيب ${item.name} في ${asset.name}`);
		...
		addInventoryMovement(database, String(installation.inventory_item_id), "return", 1, today, `تراجع عن تركيب ${installation.part_name}`);
		if (installation.replacement_of_id) {
			...
			if (oldPart.old_part_action === "return_to_stock" && oldPart.inventory_item_id) {
				...
				addInventoryMovement(database, String(oldPart.inventory_item_id), "use", 1, today, `إعادة تركيب ${oldPart.part_name} بعد التراجع عن الاستبدال`);
			}
...
async function handleLocalDataRequest(request) {
	...
	try {
		...
		if (url.pathname === "/api/local-data/query" && request.method === "POST") return Response.json({
			data: await run(await request.json()),
			error: null
		});
		if (url.pathname === "/api/local-data/hardware" && request.method === "POST") return Response.json({
			data: await runHardwareAction(await request.json()),
			error: null
		});
		...
		if (url.pathname === "/api/local-data/restore" && request.method === "POST") {
			await restore(await request.json());
...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-C_MSZ7Ed.mjs").then((m) => m.default ?? m);
