...
import { _ as Pencil, c as Trash2, h as Plus, j as CircleDot, n as Wrench, p as Search } from "../_libs/lucide-react.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/maintenance-Trss_SQ1.js
...
async function reconcileInventory(inventory, previousItems, nextItems, maintenanceId) {
	for (const adjustment of inventoryAdjustment(movementItems(previousItems), movementItems(nextItems))) {
		...
		if (nextQuantity < 0) throw new Error(`الكمية المتوفرة من ${item.name} لا تكفي`);
		...
		const movement = await supabase.from("inventory_movements").insert({
			...
			note: adjustment.quantityChange > 0 ? "إرجاع بعد تعديل أو حذف سجل صيانة" : "استخدام في سجل صيانة",
...
function Maintenance() {
	...
	const removeRecord = async (maintenanceRecord) => {
		...
		try {
			await reconcileInventory(inventory, maintenanceRecord.used_items || [], [], null);
		} catch (reconcileError) {
			toast.error(reconcileError instanceof Error ? reconcileError.message : "تعذر إرجاع الكميات");
		}
...
function MaintenanceForm({ record, assets, inventory, technicians = [], close, saved }) {
	...
	const save = async () => {
		...
		try {
			await reconcileInventory(inventory, record.used_items || [], form.used_items || [], form.id || savedRecord?.id);
		} catch (error) {
			return toast.error(error instanceof Error ? error.message : "تعذر تحديث المخزون");
		}
		saved();
	...
	const remove = async () => {
		...
		try {
			...
			toast.error(error instanceof Error ? error.message : "تعذر إرجاع الكميات");
		}
		saved();
		toast.success("تم حذف سجل الصيانة");
		close();
